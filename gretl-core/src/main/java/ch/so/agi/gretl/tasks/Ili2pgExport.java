...
public abstract class Ili2pgExport extends Ili2pgOutputFileTask {
    ...
    public void exportData() {
        ...
        Config config = config(Ili2dbFlavor.POSTGIS, Ili2dbOperation.EXPORT);
        Ili2dbConfigBuilder.applyExport(config, new Ili2dbExportOptions(getExport3().getOrElse(false), getExportModels().getOrNull()));
        execute(new Ili2dbRequest(Ili2dbFlavor.POSTGIS, Ili2dbOperation.EXPORT,
                databaseSpec(), null, config, transfers, List.of(), logFilePath(), failOnException()));
    }
}
