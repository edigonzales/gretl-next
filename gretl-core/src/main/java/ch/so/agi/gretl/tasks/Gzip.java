...
public abstract class Gzip extends AbstractCoreGretlTask {
    ...
    /**
     * Keeps a downstream gzip task sensitive to a producer recreating its
     * output, even when the regenerated bytes happen to be identical.
     */
    @Input
    public Provider<String> getDataFileFingerprint() {
        return getDataFile().map(RegularFile::getAsFile).map(file -> {
            if (!file.isFile()) {
                return "missing";
            }
            try {
                return Files.getLastModifiedTime(file.toPath()) + ":" + Files.size(file.toPath());
            } catch (IOException e) {
                throw new IllegalStateException("Could not fingerprint gzip input: " + file, e);
            }
        });
    }
